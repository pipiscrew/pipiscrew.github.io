---
title: o[php android .net] Encryption - AES128 PKCS7
author: PipisCrew
date: 2017-06-24
categories: [php,android,.net]
toc: true
---
...
![Snap55](https://www.pipiscrew.com/wp-content/uploads/2016/02/Snap55.png)

warning the encrypted/decrypted values of this article, not decrypted/encrypted with 1234567890123456!!

## PHP (decrypt)

```js
//https://gist.github.com/RiANOl/1077723
<?php function="" aes128_cbc_decrypt($key,="" $data,="" $iv)="" {="" if(16="" !="=" strlen($key))="" $key="hash('MD5'," $key,="" true);="" if(16="" !="=" strlen($iv))="" $iv="hash('MD5'," $iv,="" true);="" $data="mcrypt_decrypt(MCRYPT_RIJNDAEL_128," $key,="" $data,="" mcrypt_mode_cbc,="" $iv);="" $padding="ord($data[strlen($data)" -="" 1]);="" return="" substr($data,="" 0,="" -$padding);="" }="" $text="base64_decode('4iS2mahLSlkSk1SLuWZ1qQ==');" $key="1234567890123456" ;="" $iv='1234567890123456' ;="" echo="" aes128_cbc_decrypt($key,$text,$iv);=""?>

//https://www.leaseweb.com/labs/2014/02/aes-php-mcrypt-key-padding/
<?php $crypt="base64_decode(" 4is2mahlslksk1sluwz1qq="=");"" $key="1234567890123456" ;="" $iv='1234567890123456' ;="" $enc="MCRYPT_RIJNDAEL_128;" $mode="MCRYPT_MODE_CBC;" echo="" mcrypt_decrypt($enc,="" $key,="" $crypt,="" $mode,="" $iv);="" ```=""  =""  =""?>
...
public final class AESCrypt {
    ...
    private static String bytesToHex(byte[] bytes) {
        ...
        int v;
        for (int j = 0; j < bytes.length;="" j++)="" {="" v="bytes[j]" &="" 0xff;="" hexchars[j="" *="" 2]="hexArray[v">>> 4];
...
```js
//https://gist.github.com/RiANOl/1077723
<?php function="" aes128_cbc_encrypt($key,="" $data,="" $iv)="" {="" if(16="" !="=" strlen($key))="" $key="hash('MD5'," $key,="" true);="" if(16="" !="=" strlen($iv))="" $iv="hash('MD5'," $iv,="" true);="" $padding="16" -="" (strlen($data)="" %="" 16);="" $data="" .="str_repeat(chr($padding)," $padding);="" return="" mcrypt_encrypt(mcrypt_rijndael_128,="" $key,="" $data,="" mcrypt_mode_cbc,="" $iv);="" }="" $text="test" ;="" $key="1234567890123456" ;="" $iv='1234567890123456' ;="" echo="" base64_encode(aes128_cbc_encrypt($key,$text,$iv));=""?>
```
...
origin - http://www.pipiscrew.com/?p=3762 php-android-net-encryption-aes128-pkcs7