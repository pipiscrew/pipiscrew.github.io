---
title: How true hackers write JavaScript
author: PipisCrew
date: 2018-09-05
categories: [js]
toc: true
...
```js
//src - https://news.ycombinator.com/hn.js
...
function posf (f, a) { for (var i=0; i < a.length;="" i++)="" {="" if="" (f(a[i]))="" return="" i;="" }="" return="" -1;="" }="" function="" apos="" (x,="" a)="" {="" return="" (typeof="" x="=" 'function')="" posf(x,a)="" :="" array.prototype.indexof.call(a,x)="" }="" function="" afind="" (x,="" a)="" {="" var="" i="apos(x," a);="" return="" (i="">= 0) ? a[i] : null; }
...
function kidsOf (id) {
  ...
  if (i >= 0) {
    ks = acut(trs, i + 1);
    var n = ind($(id));
    var j = apos(function(tr) {return ind(tr) <= n},="" ks);="" if="" (j="">= 0) { ks = acut(ks, 0, j) }
  }
  return ks;
}
...
``` 3;="" i++)="" {="" remel($(id).nextsibling)="" }="" remel($(id));="" fixranks();="" var="" next="(onop()" =='newest' &&="" morenext())="" ('&next=' + morenext()) : ' '="" var="" url="el.href.replace('hide'," 'snip-story').replace('goto',="" 'onop')="" ajax(newstory,="" url="" +="" next);="" ev.stoppropagation();="" return="" false;="" }=""></ 3; i++) { remel($(id).nextsibling) }
  remel($(id));
  fixranks();
  var next = (onop() == 'newest' && morenext()) ? ('&next=' + morenext()) : ''
  var url = el.href.replace('hide', 'snip-story').replace('goto', 'onop')
...
origin - http://www.pipiscrew.com/?p=14128 how-true-hackers-write-javascript