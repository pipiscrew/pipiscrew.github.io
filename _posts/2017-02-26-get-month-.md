---
title: get month working days between range (exclude weekends &| Greek holidays)
author: PipisCrew
date: 2017-02-26
categories: [php]
toc: true
...
$row_set=get($db, "select date_start, date_end from user_vacations where user_id=".$_SESSION['id']." and date_end between '{$d_start}' and '{$d_end}'",null);
...
the same + exclude GREEK holidays 
```js
<?php
	$d_start= date("Y-01-01");
	$d_end= date("Y-12-31");
...
origin - http://www.pipiscrew.com/?p=2083 php-get-month-working-days-between-range-exclude-weekends